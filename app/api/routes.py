...
from app.observability.metrics import TOKENS_GENERATED, INFERENCE_DURATION, REQUEST_LATENCY
...
async def generate_text(
    ...
    with tracer.start_as_current_span("generate_text") as span:
        ...
        try:
            # Encode prompt
            idx = torch.tensor(tokenizer.encode(request.prompt), dtype=torch.long).unsqueeze(0).to(next(model.parameters()).device)
